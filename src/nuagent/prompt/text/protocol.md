...
Two failure paths. Both come back as the `outcome`, in the same slot a successful yield uses. Neither ends the run. A diagnostic is an ordinary message you are expected to read and fix on the next turn.
